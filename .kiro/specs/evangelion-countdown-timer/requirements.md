...
本ドキュメントは、Mac用デスクトップアプリケーションとして動作するエヴァンゲリオン風カウントダウンタイマーの要件を定義します。プレゼンテーション用途を想定し、ゼロ到達後もマイナス方向へカウントダウンを継続する機能を備えます。

## 用語集

- **Timer_Application**: カウントダウンタイマーのMacデスクトップアプリケーション
- **Timer_Display**: カウントダウン時間を表示するUI要素
- **Timer_Controller**: タイマーの開始・停止・リセットを制御するコンポーネント
- **Countdown_Value**: 現在のカウントダウン値（正の値、ゼロ、または負の値）
- **Alert_Animation**: 警告感を演出するアニメーション効果
- **Time_Input**: ユーザーがカウントダウン開始時間を入力するUI要素

## 要件

### 要件 1: 起動時の時間入力画面

**ユーザーストーリー:** プレゼンターとして、アプリ起動時にプレゼンテーションの時間を入力したい。これにより、すぐにカウントダウンの準備ができる。

#### 受け入れ基準

1. WHEN Timer_Application が起動された時、THE Timer_Application SHALL 最初に時間入力画面を表示する
...
3. WHEN スタートボタンが押された時、THE Timer_Application SHALL 時間入力画面からカウントダウン画面に遷移する
4. WHEN スタートボタンが押された時、THE Timer_Controller SHALL 入力された時間でカウントダウンを開始する
...
2. WHEN 停止ボタンが押された時、THE Timer_Controller SHALL カウントダウンを一時停止する
3. WHEN 再開ボタンが押された時、THE Timer_Controller SHALL カウントダウンを再開する
4. WHEN リセットボタンが押された時、THE Timer_Controller SHALL 時間入力画面に戻る
...
### 要件 5: エヴァンゲリオン風UIデザイン

**ユーザーストーリー:** プレゼンターとして、エヴァンゲリオン風の警告感のあるデザインでタイマーを表示したい。これにより、視覚的にインパクトのあるプレゼンテーションができる。
...
### 要件 6: Macデスクトップアプリケーション

**ユーザーストーリー:** プレゼンターとして、Macのネイティブアプリケーションとしてタイマーを使用したい。これにより、安定した動作とスムーズな操作ができる。

#### 受け入れ基準

1. THE Timer_Application SHALL macOS上でネイティブアプリケーションとして動作する
2. THE Timer_Application SHALL ウィンドウのリサイズに対応し、Timer_Display を適切にスケーリングする
3. THE Timer_Application SHALL 常に最前面に表示するオプションを提供する
